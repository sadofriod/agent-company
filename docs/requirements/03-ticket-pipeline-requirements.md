...
- `PIPE-014`：系统必须提供可被 React Flow 编辑器消费的 Pipeline DAG 结构，至少覆盖节点、连线、Step 元数据和依赖关系。
- `PIPE-015`：编辑模式对 Pipeline 的拖拽和连线修改在保存前必须执行 DAG 校验、Owner 唯一性校验和依赖存在性校验。
- `PIPE-016`：执行模式必须暴露步骤级执行状态，至少包含待执行、执行中、通过、修订、中断和阻塞。
- `PIPE-017`：执行模式必须支持按 Step 查看输入、输出、Handoff、ReviewResult 和日志摘要。
...
- 用户可以通过可视化方式编辑 Pipeline DAG，并在执行态按 Step 查看状态和产物。