...
- `RUN-013`：运行时必须输出可供执行模式消费的结构化状态流，覆盖讨论节点、Pipeline Step、Review 和中断动作。
- `RUN-014`：执行模式查看的输入输出和日志必须来源于 Runtime 结构化对象或审计事件，不得依赖前端推断生成。
- `RUN-015`：执行模式必须能够基于审计事件复盘一次任务从讨论到 Pipeline 的完整路径。
...
- 执行模式可以稳定复用 Runtime 状态和审计事件展示讨论流程与 Pipeline 执行细节。